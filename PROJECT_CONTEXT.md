...
- Auto DM：已有调度入口、近期互动候选生成、pre-send/audit、审核/拦截、真实发送、安全重试、名单管理、名单审计和公开偏好中心 MVP；会写入 `auto_dm_tasks`、`auto_dm_recipient_rules`、`auto_dm_recipient_imports` 与 `activity_logs type=dm`，审批后通过 X DM API 发送并回写成功/失败，rate limit / 5xx / 网络类失败可自动或手动重试；黑名单/退订永不发送，存在白名单时仅对白名单发送；支持 CSV allowlist 导入、导入历史、名单搜索/状态筛选/批量操作、名单变更 Activity 和 `/unsubscribe/{token}` 公开退订。
...
- Auto DM：真实发送、重试队列、白名单、黑名单、CSV allowlist、导入历史、名单变更审计、名单搜索/状态筛选/批量操作、退订记录和公开退订页已接入；仍可继续增强名单统计、风险趋势和更细的风控可视化。
...
### 当前建议任务：Auto DM 运营可视化增强

目标：在名单管理闭环基础上，补齐运营视角的状态统计、风险趋势和拦截原因聚合。

建议范围：

- `/agents` 增加 allowlist / blocked / unsubscribed 统计卡片。
- 聚合最近名单变更 Activity 和导入错误原因。
- Analytics 增加 Auto DM 名单风险摘要。
- 补充 API 文档、运行说明和测试。

这类任务主要覆盖可视化和运营判断，能继续降低真实发送后的运营风险。
