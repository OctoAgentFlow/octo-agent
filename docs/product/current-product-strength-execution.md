...
| --- | --- | --- | --- |
| 1 | Core workflow UI smoke tests | Done / refining | Added `scripts/smoke-core-ui.mjs`, `npm --prefix frontend run smoke:core`, and runbook instructions for local/production route shell checks. |
...
| 4 | Creator Studio/private analytics manual import | Done / local-first | Account Intelligence stores local private analytics notes and now appends user-provided Creator Studio observations into Growth Strategy operator notes when applied. |
...
- Run `npm --prefix frontend run lint`, `npm --prefix frontend run lint:i18n`,
  `npm --prefix frontend run build`, and `npm --prefix frontend run smoke:core`
  before considering the batch ready.
