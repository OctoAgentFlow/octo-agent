...
- Posts 页面：帖子列表、创建、详情、编辑、删除、执行；空状态会引导连接账号或创建第一条内容。
...
目标：让用户在创建、定时、执行和失败排查时获得更清晰的即时反馈。

建议范围：

- 发布前校验账号状态、内容长度、定时时间和 Auto Post 开启状态。
- 失败帖子提供重试入口和失败原因摘要。
- Posts 列表增加状态筛选与定时发布提示。
- 创建页根据账号状态给出更明确的阻断提示。

### 后续优先级

1. Posts 发布体验增强：发布前校验、失败重试提示、计划发布状态反馈。
2. Settings 安全继续增强：多会话管理、安全提醒、工作区级配置。
3. Analytics 深度指标：接入真实 X impressions / engagement 等外部表现数据。
4. Auto DM 更细的投放内容转化分析与风控策略。
5. Admin 后台管理系统：用户、订单、审计、配置、财务对账和运营工具。
