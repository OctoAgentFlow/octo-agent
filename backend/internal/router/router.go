...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	dailyXQueueRunRepo := repository.NewDailyXQueueRunRepository(db)
	...
	dailyXQueueService := service.NewDailyXQueueService(dailyXQueueContextRepo, dailyXQueueRunRepo, oafBotRepo, twitterAccountRepo, contentLibraryRepo, contentDraftRepo, aiGenerationUsageRepo, oafBotFeedbackRepo, activityRepo, reviewQueueVerdictRepo, oafBotLearningRulePrefRepo, oafBotService, aiService)
