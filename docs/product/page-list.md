...
| `/daily-x-queue` | Downlined compatibility route; redirects to Content Drafts generation mode. | Frontend redirects to `/content-drafts?panel=generate&legacy_source=daily_x_queue`; the old `/api/v1/daily-x-queue/*` backend route is no longer registered by default. |
