...
### 当前建议任务：Auto DM 执行器调研与最小执行闭环

目标：把已有 Auto DM 配置从“可配置”推进到“能执行”，但需要先确认 X API 权限和风控边界。

建议范围：

- 先梳理 X API 私信权限、授权 scope、限流与合规风险。
- 后端新增 DM 执行服务的最小骨架：选取开启 Auto DM 的账号、生成候选任务、写 Activity。
- 若本地没有真实私信权限，先做 dry-run/failed-with-reason 模式，避免假成功。
- 前端在 Agents/Activity 中补齐 DM 执行状态和失败原因。
- 补充 API 文档、运行说明和测试。
...
2. Auto DM 执行器：先 dry-run/权限校验，再接真实私信发送。
