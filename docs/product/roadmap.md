...
8. **Core workflow smoke tests**
   Add repeatable UI and API checks for login, dashboard, Exposure Radar, Content Drafts, Handling List, Billing, Admin health, response shapes, diagnostics, and activation readiness.
...
| Opportunity quality and learning loop | Done / refining | Hot/rising, quality tiers, manual records, result backfill, memory, strategy, people radar, weekly review, operator evidence, learning change summaries, opportunity quality gate, and repeat/slow/measure learning decisions exist. Collection Diagnostics now adds a user-facing reliability verdict for blocked, fallback, stale, sparse X views, strict filters, insufficient resampling, and threshold-gap states. Keep improving ranking quality and context intake. |
| Account Intelligence clarity | Done / refining | Current docs and product surfaces avoid assuming Creator Studio private data. Account Intelligence labels public X data, user-provided context, internal workflow data, unavailable private metrics, includes Account Growth Diagnosis before handoff to Daily Growth Desk, and supports local Creator Studio paste/file import that parses user-provided audience, country, active-time, growth, content-format, and weak-signal notes into Growth Strategy operator notes. Future work is authorized private metrics sync only after a confirmed data path exists. |
...
| Core workflow smoke tests | Done / refining | Added `scripts/smoke-core-workflows.sh` for static/prod checks, `scripts/smoke-core-ui.mjs` / `npm --prefix frontend run smoke:core` for local or production HTML-shell checks, and `scripts/smoke-core-api.mjs` / `npm --prefix frontend run smoke:api` for API health, auth-boundary, response-shape, Exposure Radar diagnostics, and activation-readiness checks. |
