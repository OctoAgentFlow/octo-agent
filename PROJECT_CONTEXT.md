...
目标：让新用户从空账号状态可以顺畅完成 X 账号绑定、自动化开启、首条内容发布或 Auto DM 配置。

建议范围：

- 在 Dashboard / Accounts / Agents / Posts 增加一致的空状态和下一步行动入口。
- 用真实账号、自动化、帖子和活动数据判断 onboarding 进度。
- 将关键路径串起来：绑定账号 -> 开启自动化 -> 创建内容 -> 查看 Activity / Analytics。
- 补充必要的 API 文档、运行说明和测试。

这类任务能直接提升用户首次使用体验，并且不会把后台运营复杂度提前暴露到用户端。

### 后续优先级

1. 用户端新手引导与空状态。
2. Posts 发布体验增强：发布前校验、失败重试提示、计划发布状态反馈。
3. Settings 安全继续增强：多会话管理、安全提醒、工作区级配置。
4. Analytics 深度指标：接入真实 X impressions / engagement 等外部表现数据。
5. Auto DM 更细的投放内容转化分析与风控策略。
6. Admin 后台管理系统：用户、订单、审计、配置、财务对账和运营工具。
7. 若后续需要独立 Agent 实体，将 `agents` 表与自动化配置建立关联。
