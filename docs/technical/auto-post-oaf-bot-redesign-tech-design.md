...
- Execution Queue：已聚合 comment / reply，后续可加入 post。
- Publishing Pipeline：统一 `publish_jobs`，支持 simulated publish 和人工真实发布灰度。
...
MVP 可以先只新增 Planner + 立即生成草稿，Content Library 后续补。
...
MVP 可以暂时不建表，先复用 Posts 作为内容来源。长期建议拆分，避免 Posts 同时承担素材库和发布队列两种语义。
...
### Phase 4：Publishing Pipeline 接入 post
