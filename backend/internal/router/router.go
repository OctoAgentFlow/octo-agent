...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	billingService := service.NewBillingService(userRepo, billingOrderRepo, twitterAccountRepo, oafBotRepo, cfg)
	oafBotService := service.NewOAFBotService(oafBotRepo, twitterAccountRepo, userRepo, aiService)
