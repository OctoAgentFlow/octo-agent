...
- Admin 页面：位于 `/admin`，owner/admin 可访问；展示用户、订单、活动、配置健康摘要，并支持用户角色/状态管理。
...
- 配置加载：`APP_ENV` 选择环境，`APP_SERVICE=api/admin` 选择 `backend/configs/config.<env>.api.yaml` 或 `backend/configs/config.<env>.admin.yaml`；未设置 `APP_SERVICE` 时兼容读取 `backend/configs/config.<env>.yaml`。
...
- Admin：`/api/v1/admin/overview`、`/api/v1/admin/users`、`PATCH /api/v1/admin/users/:id`，支持后台概览、用户列表、角色/状态管理；包含 owner/admin 权限保护、最后 owner 保护和禁止停用自己。
- Email：本地开发默认使用 Local sender，验证码会写入日志并在 local API 响应中返回；生产建议使用 Resend，SES 仍保留为可选兼容 provider。
- Deploy：当前采用 `scripts/` 下的脚本化四服务部署；暂不使用 Docker、Docker Compose、systemd 或项目内 Nginx 模板。

## 3. 未完成模块

- Auto DM：真实发送、重试队列、白名单、黑名单、CSV allowlist、导入历史、名单变更审计、名单搜索/状态筛选/批量操作、运营可视化、退订记录和公开退订页已接入；仍可继续增强更细的内容转化与风控策略。
- Admin：后台管理 MVP 已接入；后续仍需继续扩展订单明细运营、对账导出、系统配置编辑和更完整的操作审计检索。
...
2. 检查 `backend/configs/config.local.yaml` 中的 MySQL DSN、邮件、X OAuth、Billing 配置。本地默认 `email.provider=local`，无需真实邮件服务即可完成验证码闭环。
3. 启动 API 服务：
...
2. 本地脚本会按前端角色注入 API 地址：用户前端连 `http://localhost:10001/api/v1`，后台前端连 `http://localhost:10002/api/v1`。
...
`admin-front-local` 会设置 `NEXT_PUBLIC_FRONTEND_ROLE=admin`，打开根路径会进入 `/admin`；用户端 `api-front-local` 会设置 `NEXT_PUBLIC_FRONTEND_ROLE=api`。
...
- `APP_ENV`
  ...
  - 本地建议值：`local`。
- `APP_SERVICE`
  - 用途：选择后端服务配置文件，`api` 加载 `config.<APP_ENV>.api.yaml`，`admin` 加载 `config.<APP_ENV>.admin.yaml`。
  - 未设置时：兼容加载旧的 `config.<APP_ENV>.yaml`。
...
- `mysql.data_source`
- `log.*`
- `email.provider` / `email.local.*` / `email.resend.*` / `email.ses.*`
...
- Admin 页面 -> `/admin/overview`、`/admin/users`、`PATCH /admin/users/:id`
