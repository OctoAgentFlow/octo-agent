...
当前建议默认使用 review。autopilot 会生成 `ready_to_publish`，测试环境仍通过 Publishing Pipeline 的 simulated publish 或人工真实发布灰度，不直接大规模真实发推。
...
当前 Auto Post 不要求用户每次都手动创建一条帖子。手动创建只是内容来源之一，主工作台是 `/auto-post` 的 Planner + Content Library。
...
当前已做基础 normalized text hash 去重和近期内容参考。Embedding 语义去重放后续。
...
### MVP 1：Auto Post Planner 最小闭环（已落地）
...
### MVP 4：Publishing Pipeline 接入 post（已落地）

范围：

- `publish_jobs.source_type=post`。
- Auto Post ready_to_publish / approved 后创建 publish job。
- simulated publish 回写 Auto Post draft 状态。
- 手动真实发布灰度支持 `PublishPost`。
