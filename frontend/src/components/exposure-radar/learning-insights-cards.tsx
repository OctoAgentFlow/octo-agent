...
export function LearningChangeSummaryCard({ rows }: { rows: LearningImpactRow[] }) {
  const { t } = useT();
  return (
    <div className="rounded-2xl border border-[#1d9bf0]/20 bg-[#08131f] p-4">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-[#e7e9ea]">{t("exposureRadar.learningPanel.changeTitle")}</p>
          <p className="mt-1 text-xs leading-5 text-[#8b98a5]">{t("exposureRadar.learningPanel.changeDescription")}</p>
        </div>
        <span className="inline-flex w-fit rounded-full border border-[#1d9bf0]/25 bg-black px-2.5 py-1 text-[11px] font-semibold text-[#8ecdf8]">
          {t("exposureRadar.learningPanel.changeBadge")}
        </span>
      </div>
      <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
        {rows.length ? rows.map((row) => (
          ...
          <p className="rounded-xl border border-dashed border-[#2f3336] px-3 py-6 text-center text-xs text-[#71767b] md:col-span-2 xl:col-span-4">{t("exposureRadar.learningPanel.changeEmpty")}</p>
        )}
      </div>
    </div>
  );
