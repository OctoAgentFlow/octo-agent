...
| Posts | `/posts` | real | `GET/POST/PUT/DELETE /posts`、`POST /posts/:id/execute`；服务端 **每分钟** 调度 `scheduled` 帖子（需 Auto Post 开启）；空状态引导连接账号或创建第一条内容。 |
...
1. **Posts 发布体验增强**：补发布前校验、失败重试提示和计划发布反馈。
2. **Settings 安全继续增强**：后续再补多会话管理、安全提醒和更完整的工作区级配置。
3. **Analytics 深度指标**：后续再接真实 X impressions / engagement 等外部表现指标。
4. **Auto DM 转化与风控增强**：继续细化内容转化分析和投放安全策略。
