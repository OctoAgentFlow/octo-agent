...
- 支持：
  - 手动生成单条内容草稿。
  - run-now 手动触发一次生成规则逻辑。
  - API scheduler 到点扫描 enabled planner 生成草稿。
  - 生成时读取 X 账号绑定的 OAF Bot、人设、语言配置和内容素材。
  - AI 用量记录 `scene=auto_post`。
  - 草稿进入 Handling List，`source_type=post` 可进入 Publishing Pipeline。
- 当前不做真实自动发 X。
...
- Auto Post 新闭环优先使用 `/content-drafts`，不要把 `/posts/create` 当作 Planner 主入口。
