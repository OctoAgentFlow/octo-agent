...
| Analytics | `/analytics` | real（MVP+） | `GET /analytics/overview?range=7d|30d&account_id=...` 聚合账号级活动趋势、自动化拆分、失败原因、待处理项与内容状态，并跳转 Activity 排查。 |
...
2. **Analytics**：继续扩展账号对比、转化指标与更细的内容表现指标。
