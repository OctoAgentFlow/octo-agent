...
| Automations | loading / error / retry、Auto Comment retry toast | `frontend/src/app/(dashboard)/automations/page.tsx`、`frontend/src/app/(dashboard)/auto-comments/page.tsx` | `automation.loading.*`、`automation.error.title`、`automation.comment.retry*`、`common.retry` |
...
- `/auto-post`：Planner、Content Library、runs、drafts、run-now 文案必须继续走 `autoPost.*` / `contentLibrary.*` key。
- `/execution-queue`：post/comment/reply 的 type/status/execution_mode/publish status 不能直接展示后端枚举值。
- `/publishing`：dry-run、real publish、disabled reason、cooldown、daily limit 等提示必须走语言包。
- `/oaf-bots`：推荐职业、行业、性格、话题、语言配置、示例生成 scene 继续使用稳定 value + labelKey。
