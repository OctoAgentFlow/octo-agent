...
- Auto DM：已有调度入口、近期互动候选生成、pre-send/audit、审核/拦截、真实发送和安全重试 MVP；会写入 `auto_dm_tasks` 与 `activity_logs type=dm`，审批后通过 X DM API 发送并回写成功/失败，rate limit / 5xx / 网络类失败可自动或手动重试。
...
- Auto DM：真实发送和重试队列已接入；仍可继续增强收件人白名单/黑名单、退订记录、导入来源和更细的风控可视化。
...
### 当前建议任务：Auto DM 退订/名单管理

目标：在真实发送与 retry queue 基础上，补齐更明确的收件人准入和退出机制。

建议范围：

- 增加 Auto DM 收件人白名单、黑名单和退订记录表。
- 候选生成时跳过黑名单/退订用户。
- `/agents` 增加名单状态、退订/拉黑入口。
- 支持从 CSV 或账号互动来源导入白名单。
- 补充 API 文档、运行说明和测试。
