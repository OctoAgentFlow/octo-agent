...
func main() {
	...
	if err != nil {
		alert.NotifySync(nil, alert.Event{
			...
			Message:  "Backend API failed to connect to MySQL during startup.",
			Error:    err,
		})
	...
	if err := database.AutoMigrate(db); err != nil {
		alert.NotifySync(nil, alert.Event{
			...
			Message:  "Backend API failed during database auto migration.",
			Error:    err,
		})
	...
	if err := r.Run(cfg.API.Address()); err != nil {
		alert.NotifySync(nil, alert.Event{
			...
			Message:  "Backend API HTTP server failed to run.",
			Error:    err,
		})
