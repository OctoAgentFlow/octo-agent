...
- 已接入：Auto Comment、Auto Reply、Auto Post
- 预留：Auto DM
...
1. 用户将 Auto Comment、Auto Reply 或 Auto Post 设置为全托管。
...
6. `real_publish_enabled=true` 且 `dry_run=false` 后，人工触发 `publish-now` 才会调用真实 X Publisher Adapter。
