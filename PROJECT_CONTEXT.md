...
- Auto DM：已有调度入口、近期互动候选生成、pre-send/audit、审核/拦截与真实发送 MVP；会写入 `auto_dm_tasks` 与 `activity_logs type=dm`，审批后通过 X DM API 发送并回写成功/失败。
...
- Auto DM：真实发送 MVP 已接入；仍可继续增强收件人白名单/黑名单、退订记录、重试队列、失败恢复和更细的风控可视化。
...
### 当前建议任务：Auto DM 安全增强与重试队列

目标：在真实发送 MVP 基础上，补齐更可控的发送安全边界和失败恢复能力。

建议范围：

- 增加 Auto DM 收件人白名单/黑名单与退订记录。
- 将 X API rate limit / permission / recipient closed DM 等失败分级。
- 为可恢复失败增加 retry-after 与手动重试入口。
- 在 `/agents` 展示今日发送量、每小时限制和失败分类。
- 补充 API 文档、运行说明和测试。

这类任务覆盖 model、service、repository、前端队列状态和文档，能降低真实发送后的运营风险。
