...
- Billing 页面：订阅、套餐、支付方式、下单、支付状态轮询、订单历史、异常订单补交 tx hash、对账筛选、人工审核和退款状态标记。
...
- Billing：套餐、支付方式、订单创建、订单列表、订单查询、用户补交 tx hash、自动过期、失败原因记录、链上 webhook 确认、对账/审核/退款运营状态和手动处理接口。
- Email：AWS SES 邮件发送。
...
- Billing 扩展：当前已支持下单、订单历史、用户补交 tx hash、异常原因记录、自动过期、链上确认、对账筛选、人工审核和退款状态标记；后续仍需补后台角色权限隔离、操作人审计和真实退款执行流程。
...
- Billing 页面 -> `/billing/subscription`、`/billing/plans`、`/billing/payment-methods`、`/billing/orders`、`/billing/orders/:id/confirm`、`/billing/orders/:id/ops-action`
...
- 计费订单、异常校验原因、最近检查时间、对账/审核/退款运营状态和已确认链上交易哈希防重复。
...
### 当前建议任务：Billing 运营权限隔离

目标：在当前对账、退款和人工审核 MVP 基础上，补齐真正运营后台所需的权限和审计边界。

建议范围：

- 将 `/billing/orders/:id/ops-action` 从“当前用户订单处理”升级为 owner/admin 角色可见。
- 为审核/退款动作记录操作人、动作类型、前后状态和时间线。
- 将退款标记与后续真实退款执行器或人工凭证上传分开。
- 补充 API 文档、运行说明和测试。

这类任务能降低真实收款后的误操作风险，优先级高于继续加展示型指标。

### 后续优先级

1. Billing 运营权限隔离与操作审计。
2. 修正前端 `.env.example` 与本地后端端口不一致的问题。
3. Settings 安全和工作区级配置。
4. Billing 真实退款执行凭证与财务导出。
5. Analytics 深度外部表现指标。
