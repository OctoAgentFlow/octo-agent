...
- Auto DM：已有调度入口、近期互动候选生成、pre-send/audit、审核/拦截、真实发送、安全重试、名单管理和公开偏好中心 MVP；会写入 `auto_dm_tasks`、`auto_dm_recipient_rules` 与 `activity_logs type=dm`，审批后通过 X DM API 发送并回写成功/失败，rate limit / 5xx / 网络类失败可自动或手动重试；黑名单/退订永不发送，存在白名单时仅对白名单发送；支持 CSV allowlist 导入和 `/unsubscribe/{token}` 公开退订。
...
- Auto DM：真实发送、重试队列、白名单、黑名单、CSV allowlist、退订记录和公开退订页已接入；仍可继续增强导入历史、名单变更审计和更细的风控可视化。
...
### 当前建议任务：Auto DM 名单审计增强

目标：在导入与偏好中心基础上，补齐批量导入历史、名单变更审计和更完整的管理视图。

建议范围：

- 记录每次导入批次、成功/失败行数和失败详情。
- 名单变更写入 Activity 或独立 audit log。
- `/agents` 增加完整名单表格、搜索、状态筛选和批量操作。
- 支持互动来源一键转 allowlist。
- 补充 API 文档、运行说明和测试。

这类任务覆盖 model、service、repository、前端队列状态和文档，能继续降低真实发送后的运营风险。
