...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	autoReplyService := service.NewAutoReplyService(twitterAccountRepo, automationRepo, activityRepo, replyReservationRepo, userRepo, autoReplyDraftRepo, oafBotRepo, aiGenerationUsageRepo, aiService)
	...
	reviewQueueService := service.NewReviewQueueService(autoCommentTaskRepo, autoReplyDraftRepo, oafBotRepo, twitterAccountRepo)
