...
- `oaf_bots.twitter_account_id` 是单值字段，因此一个 OAF Bot 最多绑定一个 X 账号。
- 当前没有 inactive/archive 状态，所有现存 OAF Bot 均视为 active。
- 同一用户下，一个 `twitter_account_id` 同一时间最多只能被一个 active OAF Bot 占用。
- 服务层在创建和编辑时执行冲突校验；历史数据若存在重复，按最新 ID 查询用于生成链路，后续应先清理数据再考虑数据库唯一索引。
- 本阶段不引入 Bot 多账号绑定表。
- 多账号复用人设通过未来 Bot Template / Clone 实现。
...
- 如果该 `twitter_account_id` 已绑定其他 active OAF Bot，返回 `409` 和 `error_code=oaf_bot_twitter_account_already_bound`，提示先解绑或更换账号。

生成链路：

- Auto Post 按 `twitter_account_id` 查询绑定的 OAF Bot。
- Auto Reply 按 `twitter_account_id` 查询绑定的 OAF Bot。
- Auto Comment 按 `twitter_account_id` 查询绑定的 OAF Bot。
- Auto DM 当前仍使用既有 DM 任务生成逻辑；后续接入 OAF Bot 时也必须按 `twitter_account_id` 查询账号绑定的 OAF Bot，不允许一个 Bot 直接驱动多个账号。
