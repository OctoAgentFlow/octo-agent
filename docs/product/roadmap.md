...
| Billing | `/billing` | real（MVP+） | `GET /billing/subscription|plans|payment-methods`；**BEP20** 下单、轮询、用户补交 tx hash、自动过期、失败原因记录；链上确认 `POST /billing/webhooks/onchain`（Header 密钥）；支付记录支持 owner/admin 对账筛选、人工审核、退款状态标记和操作审计。 |
...
1. **Billing 真实退款执行凭证与财务导出**：在已隔离的 owner/admin 操作基础上，补退款凭证、CSV 导出和财务对账报表。
