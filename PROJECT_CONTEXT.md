...
- Activity 页面：活动日志列表，支持类型、状态、时间范围、账号和失败原因筛选。
- Billing 页面：订阅、套餐、支付方式、下单、支付状态轮询与订单历史。
- Posts 页面：帖子列表、创建、详情、编辑、删除、执行。
- Analytics 页面：真实接口驱动的活动概览、7/30 日趋势、账号筛选、自动化拆分、失败原因、待处理项与内容状态；可跳转 Activity 排查。
...
- Activity：活动日志分页列表、时间范围筛选、账号筛选、失败原因筛选。
- Analytics：7/30 日活动窗口、账号筛选、自动化拆分、失败原因、待处理项、内容状态聚合。
...
- Analytics 扩展：当前支持 7/30 日窗口、账号维度、失败原因、待处理项和 Activity 排查跳转，账号对比、转化指标、内容效果指标仍可继续补齐。
...
- 账号对比：在已有单账号筛选基础上，增加多账号横向对比。
- 内容效果：增加发布成功率、内容失败趋势、待审核/队列变化。
- 转化指标：从活动执行量走向互动/转化数据。
- 补充 API 文档和后端测试。
...
2. 扩展 Analytics 的账号对比、内容效果和转化指标。
