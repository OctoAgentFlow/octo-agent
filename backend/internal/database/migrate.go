...
func AutoMigrate(db *gorm.DB) error {
	if err := db.AutoMigrate(
		...
		&model.TrendTopic{},
		&model.ExposureTweetSignal{},
		&model.ExposureRadarManualRecord{},
		&model.ExposureRadarGrowthStrategy{},
		&model.ExposureRadarPeopleNote{},
		&model.TrendFeedback{},
...
func ApplyTableComments(db *gorm.DB) error {
	modelComments := []struct {
		...
		{&model.DailyXQueueContext{}, "Daily X Queue手动账号上下文"},
		{&model.DailyXQueueRun{}, "Daily X Queue生成运行记录"},
		{&model.DailyXQueueRunItem{}, "Daily X Queue运行条目记录"},
		...
		{&model.AutoCommentScanLedger{}, "Auto Comment目标扫描账本"},
		...
		{&model.ReviewQueueFeedbackIssueVerdict{}, "审核队列反馈问题判定记录"},
		{&model.OAFBotLearningRulePreference{}, "OAF Bot学习规则偏好"},
