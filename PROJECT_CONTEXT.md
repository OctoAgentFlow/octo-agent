...
- Analytics 页面：真实接口驱动的活动概览、7/30 日趋势、账号筛选、自动化拆分与内容状态。
...
- Activity：活动日志分页列表。
- Analytics：7/30 日活动窗口、账号筛选、自动化拆分、内容状态聚合。
...
- Analytics 扩展：当前支持 7/30 日窗口和账号维度，转化指标、失败原因、内容效果指标仍可继续补齐。
...
2. 扩展 Analytics 的失败原因、账号对比和转化指标。
