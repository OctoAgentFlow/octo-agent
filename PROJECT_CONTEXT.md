...
- Auto DM：已有调度入口和 dry-run/capability-check 执行服务，会写入 `activity_logs type=dm` 的待审核或失败记录；真实私信发送待 X DM 权限和风控规则确认。
...
- Auto DM：已具备调度器与 dry-run/capability-check Activity 闭环，但尚未接入真实私信发送链路。
...
### 当前建议任务：Auto DM 真实发送前置能力

目标：在已有 dry-run/capability-check 基础上，补齐真实发送前必须具备的权限、收件人规则、限流和审计边界。

建议范围：

- 明确 X API 私信权限、OAuth scope、限流与合规风险。
- 设计安全收件人来源：只允许明确互动/授权/导入白名单，避免陌生人群发。
- 增加 DM 发送审计字段或独立表，记录候选、审批、发送、失败原因。
- 将 dry-run 记录升级为可审批任务，再由用户确认后发送。
- 补充 API 文档、运行说明和测试。
...
2. Auto DM 真实发送前置能力：权限、收件人规则、审批和审计。
