...
| High-risk legacy data migration plan | Done / guarded compatibility boundary | `docs/technical/high-risk-legacy-data-migration-plan.md` defines the dual-read/dual-write, backup, backfill, test, and rollback requirements before touching DB table names, JSON fields, activity keys, or AI scene values. P3 added `scripts/check-legacy-compat-contracts.sh`, wired it into `SMOKE_LEGACY_COMPAT=1 scripts/smoke-core-workflows.sh`, grouped guard output by compatibility boundary, and added an optional `Legacy Compatibility Guard` GitHub Actions workflow plus release checklist notes so routine refactors can verify these contracts remain stable without confusing preserved legacy contracts with cleanup debt. |
