...
- Billing 扩展：当前已支持下单、订单历史、用户补交 tx hash、异常原因记录、自动过期、链上确认、对账筛选、owner/admin 人工审核和操作审计；MVP 暂不支持退款流程。
...
### 当前建议任务：Billing 财务对账导出

目标：在当前 owner/admin 权限隔离和操作审计基础上，补齐 MVP 上线后排查收款问题所需的导出能力。

建议范围：

- 增加 Billing 订单与审计 CSV 导出。
- 增加按时间、用户、计划、订单状态和审核状态的财务对账报表。
- 增加异常订单导出入口，方便人工排查链上交易。
- 补充 API 文档、运行说明和测试。

这类任务能让 MVP 上线后的收款异常更容易排查，同时不引入退款承诺。
