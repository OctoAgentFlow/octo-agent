...
- Auto DM：已有调度入口、近期互动候选生成、pre-send/audit、审核/拦截、真实发送、安全重试和名单管理 MVP；会写入 `auto_dm_tasks`、`auto_dm_recipient_rules` 与 `activity_logs type=dm`，审批后通过 X DM API 发送并回写成功/失败，rate limit / 5xx / 网络类失败可自动或手动重试；黑名单/退订永不发送，存在白名单时仅对白名单发送。
...
- Auto DM：真实发送、重试队列、白名单、黑名单和退订记录已接入；仍可继续增强 CSV/互动来源导入、用户退订落地页和更细的风控可视化。
...
### 当前建议任务：Auto DM 导入与偏好中心

目标：在名单管理基础上，补齐批量导入和用户自助退订入口。

建议范围：

- 支持从 CSV 或账号互动来源导入 allowlist。
- 提供公开退订落地页或 tokenized unsubscribe endpoint。
- `/agents` 增加批量名单管理视图。
- 增加名单变更审计和导入失败详情。
- 补充 API 文档、运行说明和测试。
