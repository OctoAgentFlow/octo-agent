...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	publishingService := service.NewPublishingService(publishJobRepo, autoCommentTaskRepo, autoReplyDraftRepo, twitterAccountRepo, userRepo, activityRepo, nil)
	...
	reviewQueueService := service.NewReviewQueueService(autoCommentTaskRepo, autoReplyDraftRepo, publishJobRepo, oafBotRepo, twitterAccountRepo)
