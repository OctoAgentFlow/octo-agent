#!/usr/bin/env bash
set -euo pipefail

echo "[deprecated] test environment servers have been released. Do not use test deploy scripts; use ./scripts/deploy-backend-api-prod.sh for server deployment."
exit 1

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_DIR="$ROOT_DIR/backend"
LOG_DIR="$ROOT_DIR/logs/deploy"
PID_DIR="$ROOT_DIR/logs/pid"
LABEL="backend-api:test"
LOG_FILE="$LOG_DIR/backend-api-test.log"
PID_FILE="$PID_DIR/backend-api-test.pid"
APP_ENV_VALUE="test"
APP_SERVICE_VALUE="api"
PORT="${PORT:-11001}"
STARTUP_TIMEOUT_SECONDS="${STARTUP_TIMEOUT_SECONDS:-120}"
STARTUP_CHECK_INTERVAL_SECONDS="${STARTUP_CHECK_INTERVAL_SECONDS:-1}"

source "$ROOT_DIR/scripts/lib/deploy-common.sh"

trap 'code=$?; cmd=$BASH_COMMAND; octo_deploy_failed "$code" "$cmd"' ERR

echo "[$LABEL] root=$ROOT_DIR"
echo "[$LABEL] app=$APP_DIR"
echo "[$LABEL] APP_ENV=$APP_ENV_VALUE APP_SERVICE=$APP_SERVICE_VALUE PORT=$PORT"

mkdir -p "$LOG_DIR" "$PID_DIR"
cd "$APP_DIR"

octo_assert_config_port "$LABEL" "$APP_DIR/configs/config.${APP_ENV_VALUE}.${APP_SERVICE_VALUE}.yaml" "api" "$PORT"
octo_stop_from_pid_file "$LABEL" "$PID_FILE"
octo_ensure_port_free "$LABEL" "$PORT"

echo "[$LABEL] starting go run ./cmd/api"
nohup env APP_ENV="$APP_ENV_VALUE" APP_SERVICE="$APP_SERVICE_VALUE" \
  go run ./cmd/api >>"$LOG_FILE" 2>&1 &
echo "$!" >"$PID_FILE"

octo_wait_for_port "$LABEL" "$PORT" "$PID_FILE" "$STARTUP_TIMEOUT_SECONDS" "$STARTUP_CHECK_INTERVAL_SECONDS"
echo "[$LABEL] log: $LOG_FILE"
octo_send_deploy_alert "$LABEL" "success" "0" ""
