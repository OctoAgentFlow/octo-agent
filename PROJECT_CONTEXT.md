...
- Billing 页面：订阅、套餐、支付方式、下单与支付状态轮询。
...
- Activity：活动日志分页列表。
- Billing：套餐、支付方式、订单创建、订单查询、链上 webhook 确认。
- Email：AWS SES 邮件发送。
- Deploy 模板：Docker、Docker Compose、Nginx、部署脚本。

## 3. 未完成模块

- Analytics：前端存在 `/analytics` 页面和 `analyticsService.overview()`，但页面基本是空壳，后端未看到独立 `/analytics` 路由。
- Auto DM：已有 UI、配置类型和数据库配置行，但文档明确说明尚无调度执行器或实际私信发送链路。
- Agents API：`GET /api/v1/agents` 仍是硬编码占位，返回 `default-agent`；当前自动化控制台并不依赖该接口，而是使用 `/automations`。
- Settings：`/settings` 当前直接重定向到 `/profile`，没有独立设置能力。
- Admin：后端 admin 当前主要提供健康检查，没有完整后台管理功能。
- 合约工程：仓库内没有 `contracts/`、Hardhat、Foundry 或 Solidity 源码；只有钱包连接和 EVM 支付校验。
- Billing 扩展：当前是 MVP，下单与链上确认已具备，支付记录列表、对账、退款/异常处理等仍可继续产品化。
...
- Agents/Automations 页面 -> `/automations`、`/automations/:type`、`/automations/:type/toggle`
...
### 推荐起手任务：Analytics MVP

目标：把 `/analytics` 从空页面补成真实统计页。

建议范围：

- 后端新增 `GET /api/v1/analytics/overview`。
- 从 `posts` 和 `activity_logs` 聚合基础指标，例如发布数、成功执行数、失败数、最近 7 天活动量。
- 前端 `/analytics` 调用真实接口并展示 3-5 个统计卡片。
- 补充 API 文档和少量后端测试。

这个任务覆盖 controller、service、repository、DTO、前端 service、页面渲染，但业务风险低，适合作为熟悉项目的第一步。

### 后续优先级

1. 修正前端 `.env.example` 与本地后端端口不一致的问题。
2. 完成 Analytics MVP。
3. 明确 `GET /api/v1/agents` 的去留：要么接入真实 `agents` 表，要么删除/隐藏占位接口。
4. 补齐 Settings 页面需求，避免 `/settings` 只是重定向。
5. 强化 Billing：支付记录列表、异常订单处理、对账、Webhook 安全校验审计。
