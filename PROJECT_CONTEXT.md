...
### 当前建议任务：Settings 通知配置

目标：把用户可控通知偏好补齐，为后续失败提醒、订阅提醒和自动化运行提醒打基础。

建议范围：

- 通知渠道偏好：邮件、产品内提醒、关键失败提醒开关。
- 通知事件偏好：自动化失败、订阅即将过期、支付异常、待审核内容。
- 后端持久化：先落在用户设置或独立 settings 表，避免只存 localStorage。
- 前端 Settings 页面：使用真实 API 读写，而不是只有本地语言偏好。
- 补充 API 文档和后端测试。
...
2. 扩展 Settings 的通知、安全和工作区级配置。
3. Billing 对账、退款/人工审核与支付筛选。
4. 扩展 Analytics 的内容效果和转化指标。
