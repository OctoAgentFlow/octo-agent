...
| 服务 | 启动命令 | 实际监听 | 结果 | 证据 |
| --- | --- | --- | --- | --- |
| 用户 API | `make api-local` | `http://localhost:10001` | 通过 | `/Users/wangxintong/CursorProjects/octo-agent/Makefile:20` 设置 `APP_ENV=local APP_SERVICE=api`；进程命令为 `cd backend && APP_ENV=local APP_SERVICE=api go run ./cmd/api`；`/Users/wangxintong/CursorProjects/octo-agent/backend/configs/config.local.api.yaml:1` 配置 `api.port: 10001` |
| Admin API | `make admin-api-local` | `http://localhost:10002` | 通过 | `/Users/wangxintong/CursorProjects/octo-agent/Makefile:23` 设置 `APP_ENV=local APP_SERVICE=admin`；进程命令为 `cd backend && APP_ENV=local APP_SERVICE=admin go run ./cmd/admin`；`/Users/wangxintong/CursorProjects/octo-agent/backend/configs/config.local.admin.yaml:1` 配置 `admin.port: 10002` |
...
配置拆分加载规则证据：`/Users/wangxintong/CursorProjects/octo-agent/backend/internal/config/config.go:270` 中 `configFilePath(env, service)` 会在 `APP_SERVICE=api` 时加载 `configs/config.local.api.yaml`，在 `APP_SERVICE=admin` 时加载 `configs/config.local.admin.yaml`。

观察项：`/Users/wangxintong/CursorProjects/octo-agent/backend/internal/config/config.go:146` 会读取 `configs/.env`，并在 `/Users/wangxintong/CursorProjects/octo-agent/backend/internal/config/config.go:227` 开始用环境变量覆盖邮箱配置。本次本地环境存在 `EMAIL_PROVIDER=resend`，所以邮箱发送链路实际使用 Resend；接口仍返回验证码，原因是 YAML 中保留了 `email.local.expose_code: true`。
...
日志摘要：
- 用户 API 日志 `/Users/wangxintong/CursorProjects/octo-agent/backend/logs/api.log` 记录 `/health` 返回 200。
- Admin API 日志 `/Users/wangxintong/CursorProjects/octo-agent/backend/logs/admin.log` 记录 `/health` 和 `/admin/health` 返回 200。
