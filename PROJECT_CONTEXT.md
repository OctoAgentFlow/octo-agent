...
- Billing 页面：订阅、套餐、支付方式、下单、支付状态轮询、订单历史、异常订单补交 tx hash、对账筛选、owner/admin 人工审核、退款状态标记和操作审计摘要。
...
- Auth：邮箱验证码、注册、登录、刷新 token、`/users/me`、通知偏好设置；用户含 `role`（`user` / `owner` / `admin`），首个注册用户为 owner。
...
- Billing：套餐、支付方式、订单创建、订单列表、订单查询、用户补交 tx hash、自动过期、失败原因记录、链上 webhook 确认、owner/admin 对账/审核/退款运营状态、全量订单视图和手动处理审计。
- Email：AWS SES 邮件发送。
...
- Billing 扩展：当前已支持下单、订单历史、用户补交 tx hash、异常原因记录、自动过期、链上确认、对账筛选、owner/admin 人工审核、退款状态标记和操作审计；后续仍需补真实退款执行凭证和财务导出。
...
- 计费订单、异常校验原因、最近检查时间、对账/审核/退款运营状态、操作审计和已确认链上交易哈希防重复。
...
### 当前建议任务：Billing 真实退款执行凭证与财务导出

目标：在当前 owner/admin 权限隔离和操作审计基础上，补齐真实财务流程所需的凭证和导出能力。

建议范围：

- 为 `mark_refunded` 补退款 tx hash / 凭证链接 / 退款金额字段。
- 增加 Billing 订单与审计 CSV 导出。
- 增加按时间、用户、计划、退款状态的财务对账报表。
- 补充 API 文档、运行说明和测试。

这类任务能让“已退款”不只是状态标记，而是可追溯的财务证据。

### 后续优先级

1. Billing 真实退款执行凭证与财务导出。
2. 修正前端 `.env.example` 与本地后端端口不一致的问题。
3. Settings 安全和工作区级配置。
4. Owner/Admin 用户管理入口。
