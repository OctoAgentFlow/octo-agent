...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	trendService := service.NewTrendService(trendTopicRepo, exposureTweetSignalRepo, trendFeedbackRepo, oafBotRepo, contentDraftPlanRepo, contentLibraryRepo, cfg.XTrends).WithAutoCommentTaskRepository(autoCommentTaskRepo)
	...
	dailyXQueueService := service.NewDailyXQueueService(dailyXQueueContextRepo, oafBotRepo, twitterAccountRepo, contentLibraryRepo, contentDraftRepo, aiGenerationUsageRepo, oafBotFeedbackRepo, activityRepo, reviewQueueVerdictRepo, oafBotLearningRulePrefRepo, oafBotService, aiService)
