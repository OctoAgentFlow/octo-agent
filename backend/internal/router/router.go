...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	postService := service.NewPostService(postRepo, twitterAccountRepo, automationRepo, activityRepo, userRepo, oafBotRepo, aiGenerationUsageRepo, aiService, trendService, cfg.XPublisher)
	autoPostService := service.NewAutoPostService(twitterAccountRepo, automationRepo, autoPostPlanRepo, autoPostDraftRepo, autoPostRunRepo, contentLibraryRepo, activityRepo, userRepo, oafBotRepo, aiGenerationUsageRepo, aiService, publishingService, trendService)
