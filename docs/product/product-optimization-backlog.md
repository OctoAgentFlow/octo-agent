...
| OPT-1 | Done | `/daily-x-queue` now redirects to `/content-drafts?panel=generate&legacy_source=daily_x_queue`; internal product links were moved to Content Drafts. The old frontend service is compatibility-only, and `/api/v1/daily-x-queue/*` is no longer registered by default. |
...
| OPT-8 | Done / local import | Account Intelligence now includes a local private-analytics notes panel plus Creator Studio paste/file import for `.txt`, `.csv`, and `.json`. The parser stays in the browser, extracts audience, countries, active times, follower growth, top formats, and weak signals, and can feed structured notes into Growth Strategy without claiming API access to private Creator Studio data. |
...
| OPT-5 | P1 | Points Center | Low-value / unclear | `/points` adds referral, redemption codes, discounts, activity claims, and point risk controls. It is not in primary nav, but it can distract from product value and adds billing/support complexity. The old Daily X Queue task copy/linking has been removed or reframed as Content Draft compatibility. | Keep `/points` hidden behind Billing unless pricing/referral strategy needs it. If kept, simplify into Billing credits only: invite credit, coupon credit, and usage credit. | `/points` redirects to Billing in the current user flow; backend point activity codes keep historical `daily_x_queue` identifiers for redemption compatibility, but default and localized copy now use Content Draft wording. |
...
| OPT-8 | P1 | Account Intelligence | Local intake complete / authorized sync pending | Product labels Creator Studio private metrics as unavailable and now gives users a structured local way to paste or import private analytics notes. The remaining gap is only automatic authorized sync, which should wait until a confirmed data path exists. | Keep the local import path as the default. If X or another approved source later exposes authorized private metrics, add backend ingestion behind explicit consent and data-source labels. | `/accounts/:id` includes local `.txt/.csv/.json` import and parses audience, country, active time, follower growth, top formats, and weak signals into Growth Strategy notes. |
