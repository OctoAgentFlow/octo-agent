...
type ExposureRadarManualRecord struct {
	...
	ResultImpressionCount int64      `gorm:"comment:发布后曝光数" json:"result_impression_count"`
	ResultLikeCount       int64      `gorm:"comment:发布后点赞数" json:"result_like_count"`
	ResultReplyCount      int64      `gorm:"comment:发布后回复数" json:"result_reply_count"`
	ResultRetweetCount    int64      `gorm:"comment:发布后转发数" json:"result_retweet_count"`
	ResultQuoteCount      int64      `gorm:"comment:发布后引用数" json:"result_quote_count"`
	ResultBookmarkCount   int64      `gorm:"comment:发布后收藏数" json:"result_bookmark_count"`
	ResultNotes           string     `gorm:"size:512;comment:发布结果备注" json:"result_notes"`
	ResultScore           int        `gorm:"index;not null;default:0;comment:发布结果评分" json:"result_score"`
	ResultCheckedAt       *time.Time `gorm:"index;comment:发布结果回填时间" json:"result_checked_at,omitempty"`
