...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	dashboardService := service.NewDashboardService(userRepo, walletRepo, twitterAccountRepo, activityRepo, autoCommentTaskRepo, autoReplyDraftRepo, autoPostDraftRepo, publishJobRepo)
	automationService := service.NewAutomationService(automationRepo, userRepo, activityRepo, postRepo, autoPostPlanRepo, autoCommentTaskRepo, autoReplyDraftRepo, autoPostDraftRepo)
	...
	oafBotService := service.NewOAFBotService(oafBotRepo, twitterAccountRepo, userRepo, aiGenerationUsageRepo, oafBotFeedbackRepo, autoPostPlanRepo, contentLibraryRepo, autoPostDraftRepo, autoReplyDraftRepo, autoCommentTaskRepo, reviewQueueVerdictRepo, oafBotLearningRulePrefRepo, aiService)
	...
	trendService := service.NewTrendService(trendTopicRepo, exposureTweetSignalRepo, trendFeedbackRepo, oafBotRepo, autoPostPlanRepo, contentLibraryRepo, cfg.XTrends).WithAutoCommentTaskRepository(autoCommentTaskRepo)
	...
	reviewQueueService := service.NewReviewQueueService(autoCommentTaskRepo, autoReplyDraftRepo, autoPostDraftRepo, publishJobRepo, oafBotRepo, twitterAccountRepo, contentLibraryRepo, reviewQueueVerdictRepo, activityRepo, autoCommentService, autoReplyService, contentDraftService, publishingService)
...
func NewAdmin(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	autoPostPlanRepo := repository.NewAutoPostPlanRepository(db)
	contentLibraryRepo := repository.NewContentLibraryRepository(db)
	trendService := service.NewTrendService(trendTopicRepo, exposureTweetSignalRepo, trendFeedbackRepo, oafBotRepo, autoPostPlanRepo, contentLibraryRepo, cfg.XTrends)
