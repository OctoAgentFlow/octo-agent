...
type AutoDMTask struct {
	...
	LastInboundScanAt   *time.Time `gorm:"index;comment:最近一次入站DM扫描时间" json:"last_inbound_scan_at,omitempty"`
	InboundReplyAt      *time.Time `gorm:"index;comment:收件人首次回复时间" json:"inbound_reply_at,omitempty"`
	InboundReplyEventID string     `gorm:"size:128;comment:收件人首次回复DM事件ID" json:"inbound_reply_event_id,omitempty"`
