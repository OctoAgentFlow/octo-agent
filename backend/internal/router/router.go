...
func NewAPI(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	autoCommentService := service.NewAutoCommentService(twitterAccountRepo, automationRepo, autoCommentTargetRepo, autoCommentTaskRepo, activityRepo, userRepo, oafBotRepo, contentLibraryRepo, aiGenerationUsageRepo, aiService, publishingService)
